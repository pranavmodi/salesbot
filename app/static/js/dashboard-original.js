...
function displayCampaignActivity(activity) {
    const container = document.getElementById('campaignActivityContainer');
    if (!container) return;

    if (activity.length === 0) {
        container.innerHTML = `
            <div class="alert alert-info text-center">
                <i class="fas fa-info-circle me-2"></i>No recent activity for this campaign.
            </div>
        `;
        return;
    }
    ...
    activity.forEach(item => {
        ...
        const statusBadgeClass = item.status === 'sent' ? 'bg-success' : 'bg-warning';
        html += `
            <tr>
                <td>${date}</td>
                <td>${item.to || 'N/A'}</td>
                <td class="text-truncate" style="max-width: 200px;">${item.subject || '(No Subject)'}</td>
                <td><span class="badge ${statusBadgeClass}">${item.status || 'N/A'}</span></td>
            </tr>
        `;
    });
...
function loadCampaignActivity(campaignId) {
    ...
    fetch(`/api/campaigns/${campaignId}/activity`)
        ...
        .then(data => {
            if (data.success) {
                displayCampaignActivity(data.activity);
